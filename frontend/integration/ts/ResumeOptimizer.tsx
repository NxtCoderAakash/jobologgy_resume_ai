...
import type { Analysis, AnalyzeResult } from "./types";

/**
 * Self-contained AI Resume Optimizer feature.
 * Inline styles only — no Tailwind / global CSS required.
 */
export interface ResumeOptimizerProps {
  backendUrl: string;
  /** Return the current user's Supabase access token (JWT). */
...
const S: Record<string, React.CSSProperties> = {
  ...
  chip: (bg: string, color: string, bd: string): React.CSSProperties => ({
...
export function ResumeOptimizer({ backendUrl, getToken }: ResumeOptimizerProps) {
...
function Chips({ items, kind }: { items: string[]; kind: "missing" | "added" | "matched" }) {
  const map = {
      ...
      <div style={S.card}>
        ...
        <div style={{ ...S.grid2, marginTop: 16 }}>
          {[
            ...
            <div key={title as string} style={{ border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
